...
const featuredBrands = [
  ...
  { name: 'Audi', logo: '/img/brands/audi.svg' },
  { name: 'Toyota', logo: '/img/brands/toyota.svg' },
  { name: 'Kia', logo: '/img/brands/kia.svg' },
  { name: 'Hyundai', logo: '/img/brands/hyundai.svg' },
  { name: 'Porsche', logo: '/img/brands/porsche.svg' },
  { name: 'Nissan', logo: '/img/brands/nissan.svg' },
  { name: 'Jeep', logo: '/img/brands/jeep.svg' },
];

// Repeat the brand list enough times that a single half of the track is wider
// than any realistic viewport (ultrawide monitors included) — that's what
// removes any "empty slot" gap at the end of a lap.
const REPEATS_PER_HALF = 5;
const brandSequence = Array.from({ length: REPEATS_PER_HALF }).flatMap((_, repeatIndex) =>
  featuredBrands.map(brand => ({ ...brand, _key: `${brand.name}-${repeatIndex}` }))
);

function BrandItem({ brand }) {
  return (
    <div className="marquee-brand-item">
      <img
        ...
        draggable={false}
      />
      <span className="marquee-brand-name">{brand.name}</span>
    </div>
  );
}
...
function BrandMarquee() {
  return (
    <div className="hero-brand-strip" aria-label="Featured car brands" dir="ltr">
      <div className="hero-brand-track">
        {/* Half A */}
        <div className="marquee-brand-group">
          {brandSequence.map(brand => (
            <BrandItem brand={brand} key={`a-${brand._key}`} />
          ))}
        </div>
        {/* Half B — exact duplicate of Half A. Because both halves are
            identical in width, animating translateX(0) -> translateX(-50%)
            loops with zero gap, on any screen size, with no JS measuring. */}
        <div className="marquee-brand-group" aria-hidden="true">
          {brandSequence.map(brand => (
            <BrandItem brand={brand} key={`b-${brand._key}`} />
          ))}
        </div>
      </div>
    </div>
